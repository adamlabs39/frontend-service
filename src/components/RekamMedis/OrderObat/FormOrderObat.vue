    ...
    <CustomAccordion headerClass="bg-adameds-50">
        ...
        <template #content>
            <div class="flex flex-col gap-5 py-3">
                ...
                <DataTable :value="orderObats" class="overflow-y-scroll text-xs bg-adameds-50 max-h-[300px]" scrollable scrollHeight="flex" table-style="min-width:50 rem">
                    <Column headerClass="bg-adameds-50 font-semibold text-SM" class="max-w-[60px] text-center">
                        <template #header>
                    ...
                    <Column headerClass="bg-adameds-50" class="w-auto text-left">
                        ...
                        <template #body="slotProps">
                            <div v-if="slotProps.data.racikan">
                                {{ slotProps.data.namaRacikan }} <span v-if="slotProps.data.sirup">- Sirup</span>
                            </div>
                            <div v-else>
                                {{ slotProps.data.namaObat }}
                            </div>
                            <div class="flex gap-1.5 justify-left">
                                <CustomChip v-if="slotProps.data.racikan" :showCheckedIcon="false" label="RACIKAN"
                                    bgColor="bg-none" textColor="text-grass-300"
                                    customClass="h-5 pr-[6px] border-grass-300 " />
                                <CustomChip v-if="slotProps.data.obatPulang" :showCheckedIcon="false"
                                    label="OBAT PULANG" bgColor="bg-none" textColor="text-info-300"
                                    customClass="h-5 pr-[6px] border-info-300 min-w-[85px]" />
                                <CustomChip v-if="slotProps.data.obatKronis" :showCheckedIcon="false"
                                    label="OBAT KRONIS" bgColor="bg-none" textColor="text-sunFlower-300"
                    ...
                    <Column headerClass="bg-adameds-50" class="w-auto text-left ">
                        ...
                        <template #body="slotProps">
                            {{ slotProps.data.jumlahKonsumsi }} {{ slotProps.data.satuanDosis }}
                        </template>
                    </Column>
                    <Column headerClass="bg-adameds-50" class="w-auto text-left ">
                        <template #header>
                        ...
                        <template #body="slotProps">
                            <div class="flex justify-center gap-1.5">
                                <CustomButton label="" background-color="bg-[#3D84E5] rounded-lg"
                                    class="h-6 w-[26px] p-0" @click="handleEdit(slotProps.index)">
                                    <img src="@/assets/icons/edit.svg" alt="Edit" />
                                </CustomButton>
                                <CustomButton label="" background-color="bg-danger-300 rounded-lg"
                                    class="h-6 w-[26px] p-0" @click="deleteObat(slotProps.index)">
            ...
            <div
                ...
                <CustomButton icon="PhPlus" label="Tambah Obat" borderColor="border-adameds-300"
                    textColor="text-adameds-300" backgroundColor="bg-white" @click="handleAdd" />
                <CustomButton icon="PhPlus" label="Tambah Obat Racikan" borderColor="border-adameds-300"
                    textColor="text-adameds-300" backgroundColor="bg-white" @click="handleAddRacikan" />
            </div>

            <!-- DialogTambahObat -->
            <DialogTambahObat v-model:isDialogVisible="dialogTambahData.isVisible" :title="dialogTambahData.title"
                @add-obat="handleAddObat" />

            <!-- DialogEdit -->
            <DialogEdit v-model:isDialogVisible="dialogEditData.isVisible" :title="dialogEditData.title"
                :obatToEdit="dialogEditData.obatToEdit" @update-obat="handleEditObat" :index="selectedObatIndex" />

            <!-- DialogTambahObatRacikan -->
            <DialogObatRacikan v-model:is-dialog-visible="dialogRacikanData.isVisible"
                :title="dialogRacikanData.title" @add-obat-racikan="handleAddObatRacikan" :index="selectedObatIndex"  :obatToEdit="dialogRacikanData.obatToEdit" @after-edit-obat-racikan="handleEditObat"/>
        </template>
        <template #footer>
            <div class="flex items-end justify-end gap-3">
                <CustomButton label="Reset" textColor="text-[#9DA4B1]" backgroundColor="bg-transparent"
                    borderColor="border-2 border-[#9DA4B1]" @click="resetFormFields" />
