...
onMounted(() => {
  ...
  if (listPermissionStr) {
    ...
    listPermission.forEach((module) => {
      const listMenuFind = templistMenu.value.find((menu) => menu.title == module.module);
      if (listMenuFind) {
...
<template>
  <div class="flex justify-between gap-5 pr-5 mt-[10px] bg-adameds-300 rounded-xl max-md:flex-wrap shadow-md mx-3">
    <div class="flex justify-between gap-5 text-sm leading-5 text-white whitespace-nowrap max-md:flex-wrap h-[60px] overflow-auto">
      <div class="grid grid-cols-[max-content_max-content_max-content] gap-1 justify-center items-center px-2.5 rounded-xl shadow-sm bg-white">
        <img loading="lazy" src="../../assets/images/adameds-logo.png" class="shrink-0 self-stretch my-auto mx-1 aspect-square w-[50px] h-[50px]" />
        <div class="bg-adameds-300 w-[3px] h-[50px] my-auto rounded-md"></div>
        <img loading="lazy" src="../../assets/images/adameds.png" class="self-stretch object-cover w-[106px] my-auto shrink-0" />
      </div>
      <div class="flex gap-2.5 my-auto overflow-x-scroll">
        <div
          ...
          :class="[checkActiveTab(menu.url) ? 'text-adameds-300 font-semibold' : 'bg-opacity-30']"
        >
          <component :is="menu.icon" :weight="menu.iconWeight ? menu.iconWeight : 'fill'" class="my-auto" />
          ...
          <template #container>
            <div class="p-2.5 rounded-2xl w-[180px]">
              ...
              <div class="flex w-full gap-5 cursor-pointer">
                <CustomButton @click="logout" label="LOGOUT" full class="font-semibold" iconPos="left" icon="PhSignOut" pt:label:class="text-XS" />
              </div>
            </div>
          </template>
        </Dialog>
