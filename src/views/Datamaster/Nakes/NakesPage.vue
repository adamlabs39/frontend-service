...
<template>
  <Card pt:body:class="h-full pt-0 overflow-auto" pt:content:class="h-full overflow-auto" class="">
    ...
    <template #content>
      <DataTable :value="products" tableStyle="min-width: 50rem" class="text-xs" selectionMode="single" stripedRows
        v-model:selection="selectedNakes" @rowSelect="handleRowSelect" scrollable scrollHeight="flex">
        ...
        <Column headerClass="bg-adameds-50">
          ...
          <template #body="slotProps">
            <div class="flex items-center justify-center">
              <CustomChip :label="slotProps.data.jenis_kelamin" :show-checked-icon="false" :textColor="slotProps.data.jenis_kelamin === 'Laki-laki'
                  ? 'text-male-300'
                  : 'text-female-300'
                " :bgColor="slotProps.data.jenis_kelamin === 'Laki-laki'
                    ? 'bg-male-75'
                    : 'bg-female-75'
                  " :border-color="slotProps.data.jenis_kelamin === 'Laki-laki'
                    ? 'border-male-75'
                    : 'border-female-75'
      ...
      <TambahDataNakesDialog v-model:isDialogVisible="isTambahDataDialogVisible" :title="dialogConfig.title"
