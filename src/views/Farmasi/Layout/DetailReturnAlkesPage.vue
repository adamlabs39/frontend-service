...
const updateQtyAlkes = () => {
  totalRetur.value = 0
  DrugReturAlkesDetailPayload.value.forEach(item => {
...
const DrugReturAlkesDetailPayload = ref<any[]>([]);

// Fetch Drug Retur Detail
const fetchDrugReturDetail = async (uuid: string) => {
  UseUtilsStore.setLoading(true);
  try {
    const response = await DrugReturStore.getApiDetailAlkes(uuid);

    if (response && response.payload) {
      DrugReturAlkesDetailPayload.value = response.payload;
    } else {
      DrugReturAlkesDetailPayload.value = [];
    }
  } catch (error) {
...
const onSubmit = async () => {
  ...
  for (const item of DrugReturAlkesDetailPayload.value) {
    for (const detailItem of item.detail) {
      ...
      const newOrderAlkes = detailItem.uuid;
      
      items.push({
        order_alkes_item_uuid: newOrderAlkes,
  ...
  try {
    const response = await DrugReturStore.createApi({
      ...
      order_alkes_uuid: props.payloadAlkes.uuid,
      items: items,
...
<template>
  <CustomDialog :visible="isDialogVisible" @update:visible="updateVisibility" width="1000px">
    <template #header>Retur Alkes - {{ props.payloadAlkes.noOrderAlkes }}</template>
    <template #body>
      <div class="pt-5">
        <!-- Nama lengkap pasien -->
      ...
      <div class="grid grid-cols-1">
        <div v-for="(items, index) in DrugReturAlkesDetailPayload" :key="index" class="mt-[20px]">
              ...
              <div class="flex justify-between">
                <div class="flex">
                  ...
                  <PhArrowRight
                    ...
                    weight="bold"
                  />
                  <p class="ml-2 text-base font-bold">{{ items.availableQty }}</p>
                </div>
              </div>
            </template>
          </card>
          <div class="pt-5 mt-[-20px]">
            ...
            >
              ...
              <Column header="Terima Retur">
                <template #body="slotProps">
                  <CustomInputNumber
                    ...
                    @update:model-value="updateQtyAlkes"
                    label=""
                    class="w-[100px]"
                  >
                  </CustomInputNumber>
              ...
              <Column header="Harga">
                <template #body="slotProps">
                  <div class="text-SM">{{ formatPrice(slotProps.data.price) }}</div>
                </template>
              </Column>
              <!-- Total -->
              <Column header="Total">
      ...
      <div class="grid grid-cols-1">
        ...
        <div class="mt-[5px]">
          <CustomTextfield v-model="alasan" :showLabel="false" placeholder="Alasan Retur" />
        </div>
      </div>
