...
const importExcel = async (file: File) => {
  ...
  try {
    const response = await StockTypeStore.importApi(dataUpload);
    fetchStockType();
...
const ExportExcel = async () => {
  try {
    ...
    const { page, page_size } = StockTypeProperties.value;
    ...
    const title = ["DATAMASTER JENIS STOK"];
    const data: any[] = [];
    ...
    data.push({
      No: "No",
      Kode: "Kode Jenis Stok",
      Nama: "Nama Jenis Stok",
      Status: "Status",
    });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Datamaster Jenis Stok");
    XLSX.writeFile(workbook, `Datamaster Jenis Stok.xlsx`);
  } catch (error) {
...
const downloadExcel = async () => {
  try {
    ...
    data.push({ No: "1", Kode: "JS001", Nama: "TUNAI" });
