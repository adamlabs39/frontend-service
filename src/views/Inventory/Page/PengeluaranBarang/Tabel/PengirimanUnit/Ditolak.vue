...
const selectedDitolakPermintaan= ref<any[]>([]);;
const ditolakPermintaans = ref<any[]>([]);;
...
watch(
  ...
  (newPermintaanData) => {
    if (Array.isArray(newPermintaanData)) {
      ditolakPermintaans.value = newPermintaanData
    }
  },
  { immediate: true }
);
...
<template>
  <!-- {{ dikirimData}} -->
  <DataTable
    v-if="ditolakPermintaans.length"
    v-model:selection="selectedDitolakPermintaan"
    :value="ditolakPermintaans"
