...
const VerificationProperties = ref({
  page: 1,
  page_size: 10,
  total: 0,
});
...
const hasData = computed(
  () => VerificationPayload.value && VerificationPayload.value.length > 0
);
...
const fetchVerification = async () => {
  ...
  try {
    const response = await VerificationStore.getApi(
      ...
      VerificationProperties.value.page_size,
    );

    if (response && response.payload) {
        VerificationProperties.value.total = response.properties.total;
      VerificationPayload.value = response.payload;
    } else {
...
const handlePage = (event: any) => {
  VerificationProperties.value.page = event.page + 1;
  VerificationProperties.value.page_size = event.rows;
  fetchVerification();
};
    ...
    <Card v-if="dataBreadCrumb.length == 0" pt:body:class="h-full pt-0 overflow-auto" pt:content:class="h-full overflow-hidden" class="h-full overflow-hidden">
        ...
        <CustomAccordion :openWithHeader="false" noBorder>
          <template #header>
            <div class="flex justify-between w-full align-middle">
              <div class="flex">
                <CustomButton icon="PhArrowClockwise" class="mr-5" @click="fetchVerification"/>
                <CustomBreadCrumb
                  :home="{
      ...
      <template #footer>
        <div class="flex justify-end">
          <CustomPaginator
            :rows="VerificationProperties.page_size"
            :totalRecords="VerificationProperties.total"
