    ...
    <Card pt:body:class="h-full pt-0 overflow-auto" pt:content:class="h-full overflow-hidden" class="h-full overflow-hidden">
        ...
        <CustomAccordion :openWithHeader="false" noBorder initial-state="1">
          <template #header>
            <div class="flex justify-between w-full align-middle">
              <div class="flex">
                ...
                <div class="">
                  <p class="font-semibold text-heading text-grey-400 ml-[10px] mt-[5px]">Rekapitulasi Terapi</p>
                </div>
              </div>
            </div>
          </template>
          <template #content>
            <div class="flex mt-[10px]">
              <CustomTextfield label="Cari Pemeriksaan" prependIcon="PhMagnifyingGlass" placeholder="Cari Pemeriksaan" class="mr-5 grow" />
      ...
      <template #content>
        <DataTable v-if="itemsPasien.length" :value="itemsPasien" scrollable scrollHeight="flex" :pt="{ headerRow: 'text-SM' }">
          ...
          <Column field="pemeriksaan" header="Pemeriksaan" headerClass="bg-adameds-50">
            <template #body="slotProps">
