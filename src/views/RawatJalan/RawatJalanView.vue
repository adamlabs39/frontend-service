...
onMounted(() => {
  ...
  if (route.query.filter) {
    const filterName = route.query.filter as string;
    const selectedItem = lokasiPayload.value.find(item => item.uuid === filterName);

    if (selectedItem) {
      filter.value = { uuid: selectedItem.uuid, name: selectedItem.name };
    }
  }
});
</script>
